...
function Export-NhcVcpkgPorts {
    ...
    .PARAMETER Tag
    Creates a subdirectory under OutputDir for the export. If a non-empty string is passed, it will be used for the directory name. Otherwise, a timestamp with format "yyMMdd-hhmmss" will be used as the directory name. Note that the string must be a valid file name without '/' or '\'.

    .PARAMETER Output
    Specifies the base name for a file export. If not passed, the default generated by vcpkg is 'vcpkg-export-<timestamp>'. Ignored for raw exports.
    ...
    Exports all installed ports from '<vcpkg-root>/installed' to './<yyMMdd-hhmmss>/vcpkg-export.zip'.
    ...
    process {
        ...
        elseif ($Format -ne "raw" -and $PSBoundParameters.ContainsKey("Output")) {
        ...
        if ($Format -ne "raw" -and $PSBoundParameters.ContainsKey("Output")) {
